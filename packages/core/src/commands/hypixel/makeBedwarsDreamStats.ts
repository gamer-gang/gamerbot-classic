...
import { StatsData } from './CommandStats';
import {
  drawPrestige,
  drawRank,
  getExpForLevel,
...
export const makeBedwarsDreamStats = (data?: Player, avatar?: Image): StatsData => {
  ...
  const argHash = hash(
    { data: data.stats.Bedwars, avatar: avatar?.src },
  ...
  const image = canvas.toBuffer('image/png', { compressionLevel: 6 });

  const statsData: StatsData = [image, [`${width}x${height}`, byteSize(image.byteLength), 'png']];
